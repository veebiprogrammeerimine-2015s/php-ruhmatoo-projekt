  ...
  if($stmt->execute()) {
    ...
    $stmt->close();
	  ...
	  function getComments($profid) {
		$stmt = $this->connection->prepare("SELECT procomment.id, comment, users.code FROM procomment INNER JOIN users ON users.id = procomment.user_id WHERE DELETED IS NULL AND accepted IS NOT NULL AND procomment.pro_id = ?");
		$stmt->bind_param("i", $profid);
		$stmt->bind_result($id, $comment_db, $code);
		$stmt->execute();
